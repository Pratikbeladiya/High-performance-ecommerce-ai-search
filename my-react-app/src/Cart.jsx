export default function Cart({ cartItems, updateQuantity, removeFromCart, setView }) {
  // Use cartItems to process the subtotal accurately
  const subtotal = (cartItems || []).reduce((acc, item) => acc + (item.price * item.quantity), 0);

  return (
    <main className="w-full max-w-[100vw] px-4 sm:px-6 lg:px-8 py-12 flex-grow">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight mb-8">Shopping Cart</h1>

        {!cartItems || cartItems.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-xl p-12 text-center shadow-sm w-full">
            ...
            <button onClick={() => setView('home')} className="bg-indigo-600 text-white font-semibold px-6 py-2 rounded-xl text-sm transition hover:bg-indigo-700 cursor-pointer">
              Start Shopping
            </button>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start w-full">
            ...
            <div className="lg:col-span-2 space-y-4 w-full">
              {cartItems.map((item) => (
                <div key={item.id} className="bg-white border border-slate-200 rounded-xl p-4 sm:p-6 flex items-center gap-4 sm:gap-6 relative w-full shadow-sm">
            ...
            <aside className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm lg:sticky lg:top-24 w-full">
              ...
              <button onClick={() => setView('home')} className="w-full text-center text-xs font-bold text-slate-400 hover:text-indigo-600 mt-4 block transition cursor-pointer">
                Continue Shopping
              </button>
            </aside>
          </div>
