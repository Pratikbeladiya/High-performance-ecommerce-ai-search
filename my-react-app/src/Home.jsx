...
export default function Home({ setView, setSelectedProduct, handleSearchSubmit, openCatalogCategory, addToCart }) {
  ...
  const getCategoryCount = (category) => {
    return DUMMY_PRODUCTS.filter((product) => product.category === category).length;
  };
    ...
    <div className="flex flex-col min-h-screen bg-slate-50 text-slate-800">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 w-full">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
          {HOME_CATEGORIES.map((cat) => (
            <div 
              key={cat.name}
              onClick={() => openCatalogCategory(cat.name)}
            ...
            >
              ...
              <p className="text-xs text-slate-500 mt-1">{getCategoryCount(cat.name)} Products</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
        <div className="flex items-center justify-between mb-8">
          ...
          <button onClick={() => openCatalogCategory('All')} className="text-sm font-semibold text-indigo-600 hover:text-indigo-700 flex items-center gap-1 cursor-pointer">
            View all <i className="fa-solid fa-arrow-right text-xs"></i>
          </button>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {DUMMY_PRODUCTS.filter((product) => product.tag).slice(0, 4).map(renderProductCard)}
        </div>
      </section>

      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 mb-16 w-full space-y-14">
        {HOME_CATEGORIES.map((cat) => {
          const products = DUMMY_PRODUCTS.filter((product) => product.category === cat.name).slice(0, 4);

          return (
            <div key={cat.name}>
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-2xl font-bold tracking-tight text-slate-900">{cat.name}</h2>
                  <p className="text-sm text-slate-500 mt-1">Fresh picks from our {cat.name.toLowerCase()} collection.</p>
                </div>
                <button onClick={() => openCatalogCategory(cat.name)} className="text-sm font-semibold text-indigo-600 hover:text-indigo-700 flex items-center gap-1 cursor-pointer">
                  View category <i className="fa-solid fa-arrow-right text-xs"></i>
                </button>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
