...
export default function App() {
  ...
  const openCatalogCategory = (category = 'All') => {
    ...
    <div className="min-h-screen w-full flex flex-col bg-slate-50 font-sans text-slate-800 antialiased m-0 p-0 overflow-x-hidden">
        ...
        <div className="w-full max-w-[100vw] px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          {isLoggedIn && (
            <div className="hidden md:flex items-center gap-8 font-medium text-sm text-slate-600">
              ...
              <button onClick={() => openCatalogCategory('All')} className={`hover:text-indigo-600 transition cursor-pointer ${view === 'catalog' ? 'text-indigo-600 font-bold' : ''}`}>Catalog</button>
      ...
      <main className="w-full flex-1 flex flex-col items-center justify-start m-0 p-0">
        ...
        {isLoggedIn && view === 'search' && (
          <div className="w-full">
            <Search key={searchQuery} defaultQuery={searchQuery} setView={setView} setSelectedProduct={setSelectedProduct} addToCart={addToCart} />
          </div>
        )}
